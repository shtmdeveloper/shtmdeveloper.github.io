(function(){
		...
		function show_custom_panel(load_url,option){
			...
			$.Deferred(function(dtd){
				...
				if (isfrm == false) {
					...
					var frmw = plw - 26/*宸﹀彸padding鍊间负2*13px*/,frame = '<iframe id="'+id+'_frame" name="'+id+'_frame" frameborder="0" src="'+load_url+'" scrolling="auto" width="'+frmw+'" onload="this.height=200;$(\'.wp-iframe_beforloaded\',\'#'+id+'\').remove();initFrame(this,\''+cc+'\',\''+plh+'\');"></iframe>';
					dtd.resolve(frame);
				}
				return dtd.promise();
			}).done(function(data){
	...
	//	var wnw = $win.width(),wnh = $win.height();
		...
		//涓轰簡闃叉鎵嬫満璁块棶PC绔乏杈硅窛涓�0鐨勯棶棰橈紝鎵€浠ヨ繖閲岀洿鎺ュ啓姝诲鏋滃乏杈硅窛涓鸿礋鏁板垯鐩存帴缁欏浐瀹氫竴涓€�
		//濡傛灉杩欓噷鍘婚厤鍚堟墜鏈虹鐨勮嚜閫傚簲浠庣洰鍓嶇殑瑙掑害鏉ヨ杩樻槸寰堥毦澶勭悊鐨勶紝鍥犱负瀹炲湪涓嶅ソ鑾峰彇褰撳墠璁惧鐨勫疄闄呭彲瑙嗛潰绉�
		if(pllt < 0){pllt = 20}
		// Overlay
...
(function(window){
	...
	WDialog.prototype.open=function(){
		...
		var imgpath=relativeToAbsoluteURL('Content/SysImages/loading.gif');
...
<iframe  name="inner" width="380" height="200" src="'+this.url+'"  frameborder="no" border="0" marginwidth="0" marginheight="0" scrolling="no" allowtransparency="true"></iframe>\n\
        ...
        var adimg_201906=relativeToAbsoluteURL('Content/SysImages/close_shopcart_pc.png');
